...
const EditContestDetail = () => {
    ...
    const fetchContestAndProblems = async () => {
      try {
        ...
        const [contestResponse, problemsResponse] = await Promise.all([
          axios.get(`http://localhost:5000/api/contests/${id}`, {
        ...
        // Find the full problem objects that are in the contest
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:5000/api/contests/${id}`, data, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      if (!response) throw new Error('Failed to update contest');
      navigate('/contests');
    } catch (err) {
...
export default EditContestDetail;